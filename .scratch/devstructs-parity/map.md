...
- [AST shape](issues/04-ast-enum-shape.md) — hybrid of the three design-it-twice proposals: `Scalar|StructPtr|Embedded|Inline` variants (shared `RefProp`, no access field — warn at parse), `Ty` resolved at parse in ast.rs with one `accessor_suffix()` quirk home, Buffer validates `s`-suffix at construction, Inflector dropped, signatures per the error-model ticket.
- [Module seams](issues/05-module-seams.md) — resolved by implication: ast.rs is its own module; goldens stay at the process seam; layout = main(CLI)/parse/ast/emit/error; pub only what main names; main01.rs deleted; order = emit, parse, diagnostics, CLI adapter with goldens green throughout. **The map is now fully decided — only fog (Not yet specified) remains.**
